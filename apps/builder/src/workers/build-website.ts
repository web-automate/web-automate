...
export async function startBuildWebsiteWorker() {
  const { channel } = await getRabbitMQ();
  
  await channel.assertQueue(queueName.build, { durable: true });
  channel.prefetch(1);

  console.log(`[*] Worker listening: ${queueName.build}`);

  channel.consume(queueName.build, async (msg) => { 
    if (!msg) return;
    ...
    try {
      ...
      if (mode !== BuildTypeWebsitePayload.BUILD_WEBSITE) {
